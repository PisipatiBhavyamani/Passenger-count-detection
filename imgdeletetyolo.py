...
for image_file in image_files:
    ...
    xml_file = image_name + ".xml"  # Assume the corresponding text file has the same name with .txt extension

    # Check if the corresponding text file exists
    if not os.path.exists(os.path.join(image_directory, xml_file)):
        # Delete the image file if the corresponding text file does not exist
        os.remove(os.path.join(image_directory, image_file))
        print(f"Deleted {image_file} because {xml_file} does not exist.")